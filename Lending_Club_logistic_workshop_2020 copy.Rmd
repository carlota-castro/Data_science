...
lc_assessment<- read_csv("Assessment Data_2020.csv") %>%  #load the data 
  ...
  mutate(
    ...
    term = factor(term_months),     # turn 'term' into a categorical variable
    delinq_2yrs = factor(delinq_2yrs)) # turn 'delinq_2yrs' into a categorical variable
profit=(con_search2$table[1,1]*(((lc_assessment$installment)*(lc_assessment$term_months))/(lc_assessment$loan_amnt)-1)-con_search2$table[1,2]*0.6)/nrow(validation)*100
profit
lc_assessment<-lc_assessment%>%mutate(profit=profit)
max<-lc_assessment%>%slice_max(profit,n=200)
lc_assessment<-arrange(lc_assessment,desc(profit))
lc_assessment<-lc_assessment%>%mutate(no=1:1799,invest=ifelse(no<=200,1,0),loan_number=as.numeric(loan_number))

excel<- tibble(x=lc_assessment$loan_number,y=lc_assessment$invest)
excel
```
